...
def sidebar():
    ...
    st.sidebar.markdown(
        ...
        • Phát hiện đánh nhau / bất thường<br>
        • Phát hiện té ngã<br>
        • Cảnh báo đỏ trên video<br>
        • Xuất báo cáo
        </div>
        """,
        unsafe_allow_html=True,
...
def open_video_safely(path):
    ...
    if not cap.isOpened():
        raise RuntimeError("Không mở được video. Hãy dùng file MP4 chuẩn H.264.")
    ...
    if first_frame is None:
        cap.release()
        raise RuntimeError("Video không có frame hợp lệ hoặc codec không hỗ trợ. Hãy xuất lại video dạng MP4 H.264.")

    h2, w2 = first_frame.shape[:2]
    if width <= 0:
        width = w2
    if height <= 0:
        height = h2

    return cap, first_frame, fps, width, height, total_frames
...
def draw_dashboard(frame, state, counts, idx):
    h, w = frame.shape[:2]
    x1, y1 = 10, max(50, int(h * 0.09))
    x2, y2 = min(w - 10, 545), y1 + 100
...
def decide_state(bbox, ratio, regions, shape, abnormal_th, fight_th, fall_hold, immediate_hold, fall_streak, abnormal_streak):
    if bbox is None:
    ...
    fall_like = aspect > 1.35 and center_y > 0.52
    fighting_like = ratio > fight_th or (regions >= 2 and ratio > abnormal_th)
    abnormal_like = ratio > abnormal_th
...
def process_video(input_path, output_path, config):
    cap, first, fps, width, height, total = open_video_safely(input_path)
    ...
    draw_dashboard(first, "Normal Activity", counts, idx)
    writer.write(first)
    ...
    while True:
        ...
        bbox, ratio, regions = motion_region(prev_gray, gray, config["sensitivity"])
        state, fall_streak, abnormal_streak = decide_state(
            ...
            config["fighting_threshold"],
            config["fall_hold_frames"],
            config["immediate_hold_frames"],
            fall_streak,
            abnormal_streak,
        )
            ...
            events.append(
                {
                    ...
                    "Motion score": round(float(ratio), 4),
                    "Số vùng chuyển động": regions,
                }
            )

        if bbox is not None:
            x1, y1, x2, y2 = bbox
            cv2.rectangle(frame, (x1, y1), (x2, y2), STATES[state]["color"], 3 if state != "Normal Activity" else 2)
            draw_label(frame, state, x1, y1 - 8)

        draw_red_banner(frame, state)
        draw_dashboard(frame, state, counts, idx)
        cv2.putText(
            frame,
            f"Motion: {ratio:.3f} | Regions: {regions}",
            (18, height - 22),
        ...
        if total:
            bar.progress(min(idx / max(1, total), 1.0))
        if idx % 25 == 0:
            status.info(f"Đang xử lý frame {idx}/{total if total else '?'}")

    cap.release()
    writer.release()

    if idx == 0:
        raise RuntimeError("Video quá ngắn hoặc không đủ dữ liệu để xử lý. Hãy dùng video dài hơn 3–5 giây.")

    bar.progress(1.0)
    status.success("Xử lý video hoàn tất.")
    return counts, events


def state_html(counts):
    html = ""
...
def page_monitor():
    ...
    with left:
        uploaded = st.file_uploader("Tải video demo lên", type=["mp4", "avi", "mov", "mkv"], help="Nên dùng video MP4 H.264, dài 10–30 giây.")
        ...
        if uploaded:
            ...
            if st.button("🚀 Bắt đầu xử lý video", type="primary"):
                ...
                try:
                    counts, events = process_video(path, out, config)
                    st.session_state.counts = counts
                    st.session_state.events = events
                    st.session_state.output_path = out
                    st.session_state.history.append(
                        {
                            "Thời gian xử lý": time.strftime("%d/%m/%Y %H:%M:%S"),
    ...
    with right:
        ...
        st.markdown(state_html(st.session_state.counts), unsafe_allow_html=True)
...
def page_events():
    ...
    if st.session_state.events:
        ...
        st.download_button("⬇️ Tải danh sách sự kiện CSV", df.to_csv(index=False).encode("utf-8-sig"), "AI_School_Safety_Events.csv", "text/csv")
    else:
...
Khi phát hiện nguy cơ, hệ thống hiển thị cảnh báo màu đỏ trực tiếp trên video.
"""

    st.markdown('<div class="main-card">', unsafe_allow_html=True)
    st.markdown('<div class="page-title">📑 Báo cáo</div>', unsafe_allow_html=True)
    st.text_area("Nội dung báo cáo", report, height=360)
    st.download_button("⬇️ Tải báo cáo TXT", report.encode("utf-8-sig"), "Bao_cao_AI_School_Safety_Monitor.txt", "text/plain")
    st.markdown("</div>", unsafe_allow_html=True)
