...
APP_NAME = "AI School Safety Monitor  " "Cuộc thi sáng tạo Thanh thiếu niên, Nhi đồng 2026"


STATES = {
    "Normal Activity": {"short": "Normal", "vi": "Hoạt động bình thường", "color": (48, 166, 78), "hex": "#16A34A", "icon": "✅"},
    "Abnormal Behavior Warning": {"short": "Abnormal", "vi": "Nghi ngờ đánh nhau / hành vi bất thường", "color": (0, 0, 255), "hex": "#DC2626", "icon": "⚠️"},
    "Teacher Check Required": {"short": "Teacher Check", "vi": "Cần giáo viên kiểm tra", "color": (0, 100, 255), "hex": "#F97316", "icon": "👨‍🏫"},
    "Fall Detected": {"short": "Fall", "vi": "Phát hiện té ngã", "color": (0, 0, 255), "hex": "#EF4444", "icon": "🚨"},
    "Immediate Check Required": {"short": "Immediate", "vi": "Cần kiểm tra ngay", "color": (0, 0, 180), "hex": "#991B1B", "icon": "🆘"},
}
...
def css():
    ...
    .app-header {background:white;border:1px solid #E5E7EB;border-radius:18px;padding:18px 22px;margin-bottom:16px;box-shadow:0 8px 24px rgba(15,23,42,.05);display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;}
    .brand-title {color:#0B2E6B;font-size:30px;font-weight:900;line-height:1.1;}
    .brand-subtitle {color:#64748B;font-size:14px;margin-top:4px;}
    .top-badge {background:#EAF2FF;color:#0B2E6B;border:1px solid #BFDBFE;padding:8px 12px;border-radius:999px;font-weight:800;}
    .main-card {background:white;border:1px solid #E5E7EB;border-radius:18px;padding:18px;box-shadow:0 8px 24px rgba(15,23,42,.05);}
    .page-title {color:#0B2E6B;font-size:24px;font-weight:900;margin-bottom:12px;}
    .state-card {background:#FFF;border:1px solid #E5E7EB;border-radius:14px;padding:12px;margin-bottom:10px;display:grid;grid-template-columns:36px 1fr 48px;gap:8px;align-items:center;}
    .state-name {font-size:13px;font-weight:900;color:#111827;line-height:1.2;}
    .state-vi {font-size:12px;color:#6B7280;line-height:1.2;margin-top:2px;}
    .state-count {font-size:23px;font-weight:900;text-align:right;}
    .sidebar-help {background:#EEF6FF;color:#0B2E6B;border:1px solid #BFDBFE;border-radius:12px;padding:12px;font-size:13px;line-height:1.35;margin-top:12px;}
    .small-note {background:#FFFBEB;border:1px solid #FDE68A;color:#92400E;border-radius:12px;padding:12px;margin-top:10px;font-size:14px;line-height:1.4;}
    .stButton>button {background:#0B2E6B;color:white;border-radius:12px;border:none;font-weight:800;padding:.6rem 1rem;}
    .stDownloadButton>button {border-radius:12px;font-weight:800;}
    video {border-radius:14px;max-height:520px;object-fit:contain;background:black;}
...
def sidebar():
    ...
    st.sidebar.caption("Demo cảnh báo an toàn học đường")
    pages = ["Giám sát trực tiếp", "Sự kiện", "Cảnh báo", "Lịch sử video", "Báo cáo"]
    st.session_state.page = st.sidebar.radio("Menu", pages, index=pages.index(st.session_state.page))
    st.sidebar.markdown("""
    <div class="sidebar-help">
    <b>Chức năng chính:</b><br>
    1. Upload video demo<br>
    2. Phát hiện hành vi bất thường / đánh nhau<br>
    3. Phát hiện té ngã<br>
    4. Hiển thị cảnh báo màu đỏ trên video<br>
    5. Xuất báo cáo và video kết quả
    </div>
    """, unsafe_allow_html=True)
...
def header():
    ...
    <div class="app-header">
        <div>
            <div class="brand-title">AI School Safety Monitor</div>
...
def draw_red_banner(frame, state):
    ...
    cv2.putText(frame, text, (max(10, (w - tw) // 2), int((banner_h + th) / 2) - 4), font, scale, (255, 255, 255), thickness, cv2.LINE_AA)
...
def draw_small_dashboard(frame, state, counts, idx):
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(frame, "AI School Safety Monitor", (x1 + 10, y1 + 24), font, 0.52, (7, 46, 116), 1, cv2.LINE_AA)
    cv2.putText(frame, f"State: {state}", (x1 + 10, y1 + 49), font, 0.43, (25, 25, 25), 1, cv2.LINE_AA)
    cv2.putText(frame, f"Abnormal: {counts['Abnormal Behavior Warning']} | Fall: {counts['Fall Detected']} | Immediate: {counts['Immediate Check Required']}", (x1 + 10, y1 + 73), font, 0.40, (60, 60, 60), 1, cv2.LINE_AA)
...
def decide_state(bbox, motion_ratio, num_regions, frame_shape, abnormal_threshold, fighting_threshold, fall_hold_frames, immediate_hold_frames, fall_streak, abnormal_streak):
    if bbox is None:
...
def process_video(input_path, output_path, config):
    ...
    while True:
        ...
        state, fall_streak, abnormal_streak = decide_state(bbox, motion_ratio, num_regions, frame.shape, config["abnormal_threshold"], config["fighting_threshold"], config["fall_hold_frames"], config["immediate_hold_frames"], fall_streak, abnormal_streak)
        counts[state] += 1
        counts["total"] = frame_index
        if state != "Normal Activity":
            events.append({"Thời gian": datetime.now().strftime("%H:%M:%S"), "Frame": frame_index, "Trạng thái": state, "Mô tả": STATES[state]["vi"], "Motion score": round(float(motion_ratio), 4), "Số vùng chuyển động": num_regions})
        if bbox is not None:
            x1, y1, x2, y2 = bbox
            color = STATES[state]["color"]
        ...
        cv2.putText(frame, f"Motion: {motion_ratio:.3f} | Regions: {num_regions}", (18, height - 22), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (40, 40, 40), 1, cv2.LINE_AA)
        writer.write(frame)
        prev_gray = gray
        if total_frames > 0:
...
def state_summary_html(counts):
    ...
    for state, info in STATES.items():
        ...
        <div class="state-card" style="border-left: 6px solid {info['hex']};">
            ...
            <div><div class="state-name">{state}</div><div class="state-vi">{info['vi']}</div></div>
...
def page_monitor():
    ...
    with col_video:
        uploaded = st.file_uploader("Tải video demo lên", type=["mp4", "avi", "mov", "mkv"], help="Nên dùng video ngắn 10–30 giây để xử lý nhanh.")
        ...
        config = {"sensitivity": sensitivity, "abnormal_threshold": abnormal_threshold, "fighting_threshold": fighting_threshold, "fall_hold_frames": fall_hold_frames, "immediate_hold_frames": immediate_hold_frames}
        if uploaded is not None:
            ...
            if st.button("🚀 Bắt đầu xử lý video", type="primary"):
                output_path = os.path.join(tempfile.gettempdir(), f"AI_School_Safety_Result_{int(time.time())}.mp4")
                try:
                    counts, events = process_video(input_path, output_path, config)
                    st.session_state.counts = counts
                    st.session_state.events = events
                    st.session_state.output_path = output_path
                    st.session_state.history.append({"Thời gian xử lý": datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "Tên video": uploaded.name, "Tổng frame": counts.get("total", 0), "Bất thường": counts.get("Abnormal Behavior Warning", 0), "Té ngã": counts.get("Fall Detected", 0), "Kiểm tra ngay": counts.get("Immediate Check Required", 0)})
            ...
            if st.session_state.output_path and os.path.exists(st.session_state.output_path):
                ...
                with open(st.session_state.output_path, "rb") as f:
                    st.download_button("⬇️ Tải video kết quả", data=f, file_name="AI_School_Safety_Result.mp4", mime="video/mp4")
        else:
    ...
    with col_status:
        ...
        st.markdown('<div class="small-note"><b>Lưu ý:</b> Cảnh báo màu đỏ sẽ xuất hiện trực tiếp trên video khi phát hiện hành vi bất thường hoặc té ngã.</div>', unsafe_allow_html=True)
...
def page_events():
    ...
    if st.session_state.events:
        ...
        st.download_button("⬇️ Tải danh sách sự kiện CSV", data=csv, file_name="AI_School_Safety_Events.csv", mime="text/csv")
    else:
...
def page_report():
    ...
    st.download_button("⬇️ Tải báo cáo TXT", data=report.encode("utf-8-sig"), file_name="Bao_cao_AI_School_Safety_Monitor.txt", mime="text/plain")
    st.markdown("</div>", unsafe_allow_html=True)
